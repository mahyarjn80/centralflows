...
Most code was copied from https://github.com/zshicode/MambaStock/blob/main/mamba.py

We don't use parallel scan, as this op seemingly doesn't play well with higher-order
autodiff in PyTorch.  Thus, the Mamba is quite slow to run.
...
class Mamba(Architecture):
    ...
    def create(self, input_shape, output_dim):
        class MambaNetwork(nn.Module):
            def __init__(self, vocab_size: int, d_model : int, layers: int, binary: bool, pscan: bool, init_scale: float):
                super().__init__()
                
                self.config = MambaConfig(d_model=d_model, n_layers=layers, pscan=pscan)

                self.embed = Embedding(num_embeddings=vocab_size, embedding_dim=d_model)
                self.mamba = StackedMamba(self.config)
                # self.linear = Linear(in_features=d_model, out_features=vocab_size)
                self.norm_f = RMSNorm(d_model)
                self.linear = Linear(in_features=d_model, out_features=1 if binary else vocab_size, bias=False)
                
                self.apply(self._init_weights)
                with torch.no_grad():
                    self.linear.weight *= init_scale
            ...
            def forward(self, x):
                ...
                features = self.mamba(embeddings)
                features = self.norm_f(features)
        ...
        return MambaNetwork(vocab_size=output_dim, d_model=self.d_model, layers=self.layers,
                            binary=self.binary, pscan=self.pscan, init_scale=self.init_scale)            
